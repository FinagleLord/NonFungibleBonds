...
contract OlympusBondDepository is Ownable {
    ...
    event BondCreated( uint deposit, uint payout, uint indexed expires, uint priceInUSD );
    event BondRedeemed( address indexed recipient, uint payout, uint remaining );
    event BondPriceChanged( uint indexed priceInUSD, uint internalPrice, uint debtRatio );
    ...
    struct Terms {
        ...
        uint stakingMultiplier; // amount paid to stakers.  in thousandths of a %
    }

    // Info for bond holder
    struct Bond {
        address owner;      // the bonds owner
    ...
    enum PARAMETER { VESTING, PAYOUT, FEE, DEBT, STAKING }
    /**
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        ...
        } else if ( _parameter == PARAMETER.STAKING ) {
            terms.stakingMultiplier = _input;
        }
    }
    ...
    function deposit( 
        ...
        uint _maxPrice,
        address _depositor
    ) external returns ( uint payout, uint bondID ) {
        ...
        uint value = treasury.valueOf( address( principal ), _amount );
        ...
        // profit share is sent to stakers
        uint toStaking = payout.mul( terms.stakingMultiplier ).div( 100000 ); 

        // for privacy on block explorer
        principal.safeTransferFrom( msg.sender, address(this), _amount ); 
        // transfer to treasury
        principal.safeTransferFrom( address(this), address( treasury ), _amount ); 
        
        // get payout
        treasury.disperse( address(this), payout.add( toStaking ) ); 
        // send rewards to staking
        OHM.transfer( staking, toStaking ); 
        ...
        bonds[ bondCount ] = Bond({
            owner: _depositor,
            payout: bonds[ bondCount ].payout.add( IStaking( staking ).stake( payout, address(this), true ) ),
    ...
    function redeem( uint _bondId, address _to ) external returns ( uint payout, bool fullyVested ) {        
        ...
        if ( percentVested >= 10000 ) { 
            ...
            uint payout = bond.payout.mul( percentVested ).div( 10000 );

            // store updated deposit info
            bonds[ _bondId ] = Bond({
                payout: bond.payout.sub( payout ),
                vesting: bond.vesting.sub32( uint32( block.timestamp ).sub32( bond.lastTime ) ),
                lastTime: uint32( block.timestamp ),
    ...
    function transferBond( uint _bondId , address _recipient ) external {
        Bond memory bond = bonds[ _bondId ];
        require(msg.sender == bond.owner );
        bond.owner = _recipient;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return fixedSupply.mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = bondPrice().mul( IBondCalculator( bondCalculator ).markdown( address( principal ) ) ).div( 100 );
        } else {
            price_ = bondPrice().mul( 10 ** principal.decimals() ).div( 100 );
        }
    }
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        if ( isLiquidityBond ) {
            return debtRatio().mul( IBondCalculator( bondCalculator ).markdown( address( principal ) ) ).div( 1e9 );
        } else {
            return debtRatio();
        }
    }
    ...
    function debtDecay() public view returns ( uint decay_ ) {
        uint secondsSinceLast = uint32( block.timestamp ).sub32( lastDecay );
     ...
     */
    function percentVestedFor( address _bondId ) public view returns ( uint percentVested_ ) {
        Bond memory bond = bonds[ _bondId ];
     ...
     */
    function pendingPayoutFor( address _bondId ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _bondId );
        uint payout = bonds[ _bondId ].payout;
